...
data |>
  ggplot(aes(x = reorder(disorder, estimate_mean), y = estimate_mean, label = round(estimate_mean))) +
...
data |>
  filter(gender != "Both") |>
  # ggplot(aes(x = reorder(disorder, estimate_mean), y = estimate_mean, fill = gender, label = paste0(round(estimate_mean), "   [", round(estimate_low), ",", round(estimate_high), "]"))) +
  ggplot(aes(x = reorder(disorder, estimate_mean), y = estimate_mean, fill = gender, label = round(estimate_mean))) +
  ...
  geom_errorbar(aes(ymin = estimate_low, ymax = estimate_high),
                position = position_dodge(width = 1), width = 0.2, alpha = 0.6, linewidth = 0.9) +
  geom_text(aes(y = estimate_high + 65 , ymax = max(estimate_high * 1.09)), size =  3.3,
            hjust = ifelse((filter(data, gender != "Both") |> pull(estimate_mean) > 100), 2.2, 2.6), position = position_dodge(0.5)) +
  coord_flip() +

...
géographiques (commune de résidence). Nous évaluons ensuite, pour l’année 2023, la répartition de cet
effectif par commune de résidence et dérivons plusieurs métriques autres que
les effectifs bruts, affinant notre représentation des inégalités territoriales
en termes de besoins socio-éducatifs. Puis, sur la base des prévalences
issues du travail de Westlake et collaborateurs (2023, DOI:
10.1177/25161032231202256), nous dérivons une estimation théorique des besoins
en santé mentale des enfants et des jeunes placés, au niveau global et au
...
l’Enfance" de la DREES, disponibles en open data (Base de données brutes, Open
dataset). Après une diminution jusqu’en 2005 puis une longue période de
...
data_2002_2022_total |>
  ...
  labs(x = "Année", y = "") +
  ggtitle("Evolution du nombre d'enfants accueillis au sein de l'ASE
...
La carte suivante propose une représentation des mesures de placement pour 1000
habitants dans les communes de résidence des enfants placés, révélant là encore
d'importantes disparités territoriales. Jambville se distingue par une valeur
exceptionnellement élevée (~ 100 enfants/1000 habitants), indiquant une forte
proportion relative de placements par rapport à sa population. Des communes
comme Auffargis présente également un taux notable, soulignant des besoins
importants (~ 40 enfants/1000 habitants). Malgré leur taille modeste, certaines
communes rurales ou périurbaines, telles que Sailly, Houdan, Les Mesnuls ou
Auffreville-Brasseuil affichent des taux non négligeables (~ 10-15 enfants/1000
habitants). Les communes urbaines comme Versailles, Mantes-la-Jolie, ou
Trappes, bien que densément peuplées et touchées en effectifs absolus,
affichent des taux de placement modérés au regard de leur population totale. La
présence de taux élevés dans des communes de petite et moyenne taille peut
...
de la mesure et sa date de fin (réelle ou théorique lorsque les mesures sont en
cours).
...
Bouafle et Bullion
présentent également des durées élevées, proches de 1 500 à 2 500 jours.
Ces chiffres traduisent des parcours prolongés suggérant des situations
complexes pouvant nécessiter un accompagnement en santé mentale de longue durée.
La majeure partie des communes restantes situées aussi bien en zone urbaine,
péri-urbaines ou rurales, présentent des durées médianes plus courtes (~ 300 à
500 jours, cercles gris à violets clairs).
...
plusieurs besoins spécifique en santé mentale. Nous proposons néanmoins
...
*NB : A première vue, il peut sembler incohérent que l'effectif présentant un trouble externalisé ()spécifique) soit supérieur à l'effectif présentant tout type de troubles mentaux (aspécifique). Cela est simplement dû au fait que ces deux indices de prévalence sont obtenus à partir de corpus d'études différents. Par exemple, une étude peut renseigner un taux de prévalence spécifique (trouble externalisé) sans nécessairement renseigner un taux aspécifique (tout type de trouble), et vice versa.*
...
df_segments <- data.frame(
  ...
  scenario = data_timeseries |> filter(scenario != "historic") |> pull(scenario),
...
# geom_segment(data = df_segments |> filter(
#                !(scenario %in% c("Scénario -9.4%  (Baisse historique)",
#                                  "Scénario +15.5% (Hausse historique)"))
#              ),
#              aes(x = x1, y = y1, xend = x2, yend = y2), linetype = "dashed", linewidth = 1.1, color = "grey") +
# geom_segment(data = df_segments |> filter(
#                scenario %in% c("Scénario -9.4%  (Baisse historique)",
#                                "Scénario +15.5% (Hausse historique)")
#              ),
#              aes(x = x1, y = y1, xend = x2, yend = y2, color = disorder), linetype = "dashed", linewidth = 1.1) +
#  geom_point(data = data_timeseries,
#             aes(x = as.integer(year), y = estimate_mean, group = disorder,
#                 color = disorder), shape = 21, color = "black",
#             fill = "dark grey", size = 3) +
#  # geom_errorbar(data = data_timeseries,
#  #               aes(x = as.integer(year), ymin = estimate_low,
#  #                   ymax = estimate_high, group = disorder, color = disorder),
#  #               width = 0.2, alpha = 0.6, linewidth = 0.9) +
...
#          subtitle = "Les sections en pointillé gris correspondent aux scenarii +20 %, +10 %, +0 %, -10 % et -20 % par rapport aux effectifs de 2022.\nLes sections en pointillé coloré projettent une augmentation et une diminution des effectifs correspondant à la hausse (2017-2022 : +15,5 %) et à la baisse (2003-2008 : -9,4 %) maximales observées\nau cours de la période 2002-2022, par fenêtres de 5 ans.") +
#  theme_ipsum() +
#  theme(legend.position = "inside",legend.position.inside = c(0.15, 0.9), legend.background = element_rect()) +
#  # theme(legend.position = "inside", legend.position.inside = c(0.5, 0.8), legend.box = "horizontal", legend.spacing = unit(1, units = "in")) +
...
* différents taux d'augmentation
  ...
  - +20%
* différents taux de diminution (-10%, -20%)
...
en protection de l’enfance ont tous lieu d’être saturés, lorsqu’ils ne sont pas
